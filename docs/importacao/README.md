...
| `email` | Não | texto | E-mail de **contato** do irmão. É apenas um dado do cadastro — **não cria login** e não tem relação com o e-mail interno usado pelo Supabase Auth |
...
> **Desde 2026-09-08 o login é por CIM + senha, e não há mais autocadastro.**
> A tela de cadastro foi removida — ninguém cria a própria conta. Para dar
> acesso a um irmão, o caminho recomendado é o script de provisionamento
> (veja `scripts/README.md`), que cria o usuário, o perfil e o papel de uma vez.

Para dar acesso a um irmão importado manualmente, depois da importação:
1. O admin cria o usuário no Supabase Auth com e-mail `{cim}@lojaamordapatria.local` e senha inicial.
...
3. Marque `profiles.must_change_password = true` para forçar a troca da senha inicial.
4. Conceda o papel em `/commission/management` → aba **Acessos** (Membro / Administrador / Diretoria).

Lembre-se: quem precisa enxergar as telas de comissão (`/commission/*`) depende
de `profiles.is_director_member = true` — o papel `admin` em `user_roles`
sozinho **não** libera esse menu.
